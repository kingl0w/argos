...
Reconcile drift between .specs/tickets/, STATE.md, and git history.

Steps:
1. Inventory tickets — for each .specs/tickets/*.md file: ID, Status from frontmatter, verification status, whether it has Implementation notes.
...
5. Invoke chaos probe: bash scripts/argos-chaos-probe.sh state.
