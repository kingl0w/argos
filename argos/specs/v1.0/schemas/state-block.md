...
|-----------|--------|---------|-----------------|
| `id`      | `{ISO-8601-UTC-timestamp}-{ticket-id}[-{suffix}][-{collision-hex}]` | `2026-04-26T14:33:01Z-ARG-042`, `…-ARG-042-drift`, `…-ARG-042-3f9c2a` | Globally unique within a single STATE.md file. Duplicates raise `DuplicateIdError`. The two trailing slots are optional and additive — see the id-grammar note below. |
...
### Id grammar (optional slots)

Two optional trailing slots may appear after the ticket-id segment, in this order:

1. **Disambiguation suffix** — a writer-supplied slug matching `^[a-z0-9-]+$`, used to distinguish entries with related but distinct purposes on the same ticket (e.g. a `verified` entry and a `drift` entry written in the same cycle). Set via `argos state-append --suffix <slug>` (per ARG1-061). Optional; absent by default.
2. **Collision-retry hex** — a 6-character hex suffix (`[0-9a-f]{6}`) appended only when the otherwise-final id already exists in the file, to satisfy the global-uniqueness invariant. Generated by `argos state-append` (per ARG1-051) and not writer-controllable.

Both slots are optional and order-stable: `{timestamp}-{ticket}` (neither), `{timestamp}-{ticket}-{suffix}` (disambiguation only), `{timestamp}-{ticket}-{collision-hex}` (collision only), `{timestamp}-{ticket}-{suffix}-{collision-hex}` (both). The parser does not interpret the trailing slots — it only requires global uniqueness of the resulting string.
