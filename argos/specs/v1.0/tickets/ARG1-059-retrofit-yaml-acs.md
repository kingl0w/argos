...
- [ ] `grep -rn "import yaml" argos/specs/v1.0/tickets/` returns no matches (exits 1).
- [ ] `grep -rn "yaml.safe_load\|pyyaml" argos/specs/v1.0/tickets/` returns no matches (exits 1).
...
- [ ] ARG1-012 AC#1 (the `python -c "import yaml; yaml.safe_load(...)"` line in `argos/specs/v1.0/tickets/ARG1-012-dispatch-log-writer.md`) is rewritten to invoke `argos frontmatter-parse` against the dispatch log path and assert the same six required keys (`ticket_id`, `epic_id`, `batch_id`, `dispatched_at`, `worktree_path`, `session_id`).
